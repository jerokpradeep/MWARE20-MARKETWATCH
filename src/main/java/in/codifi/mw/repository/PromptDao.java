...
public class PromptDao {
	...
	public void loadPromptInfoData() {
		...
		Connection conn = null;
		ResultSet rSet = null;
		List<Prompt> response = new ArrayList<Prompt>();
		HazelcastConfig.getInstance().getPromptMasterv2().clear();
		try {
			conn = entityManager.getConnection();
			String query = "SELECT isin, exch,company_name, msg, type, severity, prompt FROM tbl_asm_gsm ";
			pStmt = conn.prepareStatement(query);
			...
			if (rSet != null) {
				while (rSet.next()) {
					...
					String exch = rSet.getString("exch");
					String key = (isin + "_" + exch).toUpperCase();
					result.setCategory("Surveillance");
					result.setDescription(rSet.getString("msg"));
					System.out.println(rSet.getString("msg"));
					response = HazelcastConfig.getInstance().getPromptMasterv2().get(key);
					if (response != null && response.size() > 0) {
						response = HazelcastConfig.getInstance().getPromptMasterv2().get(key);
						response.add(result);
						HazelcastConfig.getInstance().getPromptMasterv2().put(key, response);
					} else {
						response = new ArrayList<>();
						response.add(result);
						HazelcastConfig.getInstance().getPromptMasterv2().put(key, response);
