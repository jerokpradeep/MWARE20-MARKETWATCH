...
//	        marketWatch.setUpdatedBy(pDto.getUpdatedBy());	        
	...
	private List<PredefinedMwScripsEntity> getScripMW(PredefinedMwEntity data, long maxSortOrder) {
		...
		try {
			for (PredefinedMwScripsEntity scrip : data.getScrips()) {
				...
				try {
					// Check if the Redis hash contains the key
					if (jedisPool.getResource().hexists("contractMaster", cacheKey)) {
						// Retrieve the JSON data from Redis and deserialize it into a
						// ContractMasterModel
						String json = jedisPool.getResource().hget("contractMaster", cacheKey);
						ObjectMapper objectMapper = new ObjectMapper();
						ContractMasterModel model = objectMapper.readValue(json, ContractMasterModel.class);

						// Update scrip entity with the contract data
						scrip.setMwId(data.getMwId());
						scrip.setToken(model.getToken());
						scrip.setExchange(model.getExch());
						scrip.setSortOrder(maxSortOrder);

						// Add the populated scrip entity to the response list
						response.add(scrip);
					}
				} catch (Exception e) {
					e.printStackTrace(); // Consider logging this exception in your production code
				}
			}
		} catch (Exception e) {
			e.printStackTrace(); // Consider logging this exception in your production code
		} finally {
			jedisPool.close(); // Ensure the Redis connection is closed after use
		}
	...
	private PredefinedMwScripsEntity enrichWithCacheData(PredefinedMwScripsEntity scrip) {
		...
		try {
			// Check if the cache exists for the given key
			if (jedisPool.getResource().hexists("contractMaster", cacheKey)) {
				// Retrieve the cached JSON data from Redis
				String json = jedisPool.getResource().hget("contractMaster", cacheKey);
				...
				// If valid masterData is retrieved, enrich the scrip with the data
				if (masterData != null) {
			...
			// Log the error (Replace with proper logging framework in production)
			Log.error("Error enriching scrip with cache data: " + e.getMessage());
			e.printStackTrace(); // Optional: Replace with a logging mechanism in production
		} finally {
			jedisPool.close();
		}
